  ...
  describe "PATCH /update" do
    context "with valid parameters" do
      ...
      it "updates the requested canceled_trip_trip_join_request" do
        patch canceled_trip_trip_join_request_url(trip_request, :trip_id => trip.id, :id => trip_request.id), params: { trip_join_request: new_attributes }
        trip_request.reload
        expect(trip_request.stage).to eq(60)
      end
    ...
    context "with invalid parameters" do
      it "renders a successful response (i.e. to display the 'edit' template)" do
        patch canceled_trip_trip_join_request_url(trip_request, :trip_id => trip.id, :id => trip_request.id), params: { trip_join_request: invalid_attributes }
